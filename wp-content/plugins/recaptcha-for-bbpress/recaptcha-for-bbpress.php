...
function rfb_plugin_settings_link($links){
	if(is_plugin_active('bbpress/bbpress.php')){
		...
		$settings_link = '<a href="options-general.php?page=bbpress#_bbp_allow_anonymous">Settings</a>';
...
function rfb_add_recaptcha_key_field($fields){
	...
	foreach ($fields['bbp_settings_users'] as $k => $v){
		...
		if($k=='_bbp_allow_anonymous'){
			...
			$new_fields_arrray['rfb_bbp_recaptcha_site_key']['title'] = 'reCAPTCHA Site Key';
			...
			$new_fields_arrray['rfb_bbp_recaptcha_server_key']['title'] = 'reCAPTCHA Servicer Key';
			...
			$new_fields_arrray['rfb_bbp_recaptcha_registerd_user']['title'] = 'reCAPTCHA logged in User';
...
function rfb_bbp_admin_setting_callback_recaptcha_site_key() {
	...
	<br><label for="rfb_bbp_recaptcha_site_key"><a href="https://www.google.com/recaptcha/admin" target="_blank" tabindex="-1"><?php esc_html_e( 'Get reCAPTCHA v2 Api Keys', 'bbpress' ); ?></a></label>
	<?php
...
function rfb_bbp_admin_setting_callback_recaptcha_registered_users(){
	...
	<label for="rfb_bbp_recaptcha_registerd_user"><?php esc_html_e( 'Enable for logged-in users?', 'bbpress' ); ?></label>
...
function rfb_bbp_validate_recaptcha(){
	...
	if($server_key != false & $site_key != false & $server_key != false & (($is_guest_enable != false and !is_user_logged_in()) or $rfb_bbp_recaptcha_registerd_user!=false)){
		...
		if (!$resp->isSuccess()) {
			bbp_add_error( 'bbp_throw_error', __( "<strong>ERROR</strong>: reCAPTCHA is required.") );
		}
